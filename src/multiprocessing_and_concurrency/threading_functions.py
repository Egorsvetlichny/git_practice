# Multithreading - многопоточность, зависит от ОС, страдает от GIL, подходит только для IO-bound задач.
# Любая программа это минимум один процесс и один поток.
# При использовании многопоточности полезно использовать queue, pool executor,
# daemon=True, но всё зависит от разработчика!

# Плюсы:
# + Простота использования (сравнительно с asyncio)
# + Быстрота
# + Не умирает из-за одного потока (блокирующего вызова)
# Минусы:
# - Высокое потребление ресурсов
# - Плохая управляемость (сравнительно с asyncio)
# - Проблемы потоков (состояния гонки и прочее)
